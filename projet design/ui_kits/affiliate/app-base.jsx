...
const DICT = {
  fr: {
    nav: [
      ...
      { label: "Affilié", href: "#", active: true },
    ],
    install: "Rejoindre notre programme",
    menuOpen: "Ouvrir le menu",
    langLabel: { fr: "Français", en: "English" },
  ...
  en: {
    nav: [
      ...
      { label: "Affiliate", href: "#", active: true },
    ],
    install: "Join our program",
    menuOpen: "Open menu",
    langLabel: { fr: "Français", en: "English" },
...
function Flag({ k, w = 28, h = 18 }) {
  if (k === "fr") return (
    <svg viewBox="0 0 60 40" width={w} height={h} preserveAspectRatio="xMidYMid slice">
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <img src="../../assets/logo.png" alt="" style={{ width: 46, height: 46, borderRadius: 12, margin: "0 auto 14px", display: "block" }}/>
...
function Navbar() {
  ...
  const pick = (l) => { setLang(l); setLangOpen(false); };
  return (
    <div data-aff-nav style={{
      ...
      <nav style={{
        ...
        <a href="/" style={{ position: "relative", display: "inline-flex", alignItems: "center", gap: 10, textDecoration: "none" }}>
          <img src="../../assets/logo.png" alt="" style={{ width: 26, height: 26, borderRadius: 7 }}/>
          <span style={{ fontFamily: "var(--font-heading)", fontWeight: 700, fontSize: 16, letterSpacing: "-0.01em", color: "#fff" }}>Moonbundles</span>
        </a>

        {/* Desktop links */}
        <div className="aff-nav-links" style={{ position: "relative", display: "flex", alignItems: "center", gap: 28 }}>
          {t.nav.map((l, i) => (
            <a key={i} href={l.href} style={{
              color: l.active ? "#fff" : "rgba(255,255,255,.68)",
              textDecoration: "none", fontSize: 13, fontWeight: 500,
              transition: "color .2s", whiteSpace: "nowrap",
              position: "relative",
            }}
            onMouseEnter={e => { if (!l.active) e.currentTarget.style.color = "#fff"; }}
            onMouseLeave={e => { if (!l.active) e.currentTarget.style.color = "rgba(255,255,255,.68)"; }}
            >
              {l.label}
              {l.active && (
                <span style={{
                  position: "absolute", left: 0, right: 0, bottom: -6, height: 2,
                  borderRadius: 2, background: "linear-gradient(90deg, #4d7cff, #7c5cff)",
                }}/>
              )}
            </a>
          ))}
        ...
        <div style={{ position: "relative", display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <div style={{ position: "relative" }}>
            <button onClick={() => setLangOpen(v => !v)} aria-label="Change language" style={{
              display: "inline-flex", alignItems: "center", justifyContent: "center",
            ...
            >
              <span style={{ borderRadius: "50%", overflow: "hidden", lineHeight: 0, display: "inline-flex" }}>
                <Flag k={lang} w={18} h={18}/>
              </span>
            </button>
                  ...
                  {["fr", "en"].map(l => {
                    ...
                    return (
                      <button key={l} onClick={() => pick(l)} style={{
                        ...
                        background: sel ? "rgba(77,124,255,.14)" : "transparent",
                        color: sel ? "#4d7cff" : "rgba(255,255,255,.75)",
                        border: "none", cursor: "pointer",
                        fontSize: 13, fontWeight: 500, textAlign: "left",
                        fontFamily: "inherit",
                        transition: "background .15s, color .15s",
                      }}
                      onMouseEnter={e => { if (!sel) { e.currentTarget.style.background = "rgba(255,255,255,.05)"; e.currentTarget.style.color = "#fff"; } }}
                      onMouseLeave={e => { if (!sel) { e.currentTarget.style.background = "transparent"; e.currentTarget.style.color = "rgba(255,255,255,.75)"; } }}
                      >
                        <span style={{ display: "inline-flex", width: 18, height: 18, borderRadius: "50%", overflow: "hidden", lineHeight: 0, flex: "0 0 auto" }}>
                          <Flag k={l} w={18} h={18}/>
                        </span>
                        {t.langLabel[l]}
                        {sel && (
                          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" style={{ marginLeft: "auto" }}>
                            <path d="M5 12l5 5L20 7" stroke="#4d7cff" strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round"/>
                          </svg>
                        )}
                      </button>
                    );
          ...
          {/* CTA */}
          <a href="#" className="aff-nav-cta" style={{
            background: "#fff", color: "#0a1628", borderRadius: 9999,
            padding: "9px 18px", fontSize: 12.5, fontWeight: 600, textDecoration: "none",
            whiteSpace: "nowrap",
            boxShadow: "0 2px 10px rgba(0,0,0,.25)",
            transition: "box-shadow .25s, transform .25s",
          }}
          onMouseEnter={e => { e.currentTarget.style.boxShadow = "0 0 28px rgba(255,255,255,.35), 0 2px 12px rgba(0,0,0,.3)"; e.currentTarget.style.transform = "translateY(-1px)"; }}
          onMouseLeave={e => { e.currentTarget.style.boxShadow = "0 2px 10px rgba(0,0,0,.25)"; e.currentTarget.style.transform = "translateY(0)"; }}
          >{t.install} →</a>

          {/* Mobile hamburger */}
          <button
            className="aff-nav-burger"
            ...
            <span style={{ display: "block", width: 16, height: 1.5, background: "#fff", borderRadius: 2,
              transform: mobileOpen ? "translateY(3.5px) rotate(45deg)" : "none", transition: "transform .25s" }}/>
            <span style={{ display: "block", width: 16, height: 1.5, background: "#fff", borderRadius: 2,
      ...
      {/* Mobile menu */}
      {mobileOpen && (
        <div className="aff-nav-mobile" style={{
          maxWidth: 1080, margin: "10px auto 0",
          borderRadius: 20,
          border: `1px solid rgba(255,255,255,.12)`,
          background: "rgba(12,20,40,.92)",
          backdropFilter: "blur(22px) saturate(180%)",
          ...
          <div style={{ display: "flex", flexDirection: "column" }}>
            {t.nav.map((l, i) => (
              <a key={i} href={l.href} onClick={() => setMobileOpen(false)} style={{
                padding: "12px 14px", textDecoration: "none",
                color: l.active ? "#fff" : "rgba(255,255,255,.75)",
                fontSize: 15, fontWeight: 500, borderRadius: 10,
                background: l.active ? "rgba(77,124,255,.14)" : "transparent",
              }}>{l.label}</a>
            ))}
            <a href="#" style={{
              marginTop: 10, textAlign: "center",
              background: "#fff", color: "#0a1628", borderRadius: 9999,
              padding: "12px 18px", fontSize: 14, fontWeight: 600, textDecoration: "none",
            }}>{t.install} →</a>
          </div>
        </div>
      )}
